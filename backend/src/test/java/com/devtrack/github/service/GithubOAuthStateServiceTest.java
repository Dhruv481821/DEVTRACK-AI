...
class GithubOAuthStateServiceTest {
  ...
  void createState_storesUserIdWithATtl() {
    ...
    verify(valueOperations)
        .set("github-oauth-state:" + state, userId.toString(), Duration.ofMinutes(10));
  }
  ...
  void consumeState_withValidState_returnsUserIdAndDeletesIt() {
    ...
    when(valueOperations.get("github-oauth-state:" + state)).thenReturn(userId.toString());

    Optional<UUID> result = stateService.consumeState(state);

    assertThat(result).contains(userId);

    verify(redisTemplate).delete("github-oauth-state:" + state);
  }

  /**
   * The scenario that matters most for this class — an expired or already-used state must not
   * silently succeed.
   */
  @Test
  void consumeState_withUnknownOrExpiredState_returnsEmptyAndDoesNotDelete() {
    ...
    Optional<UUID> result = stateService.consumeState("never-issued-or-expired-state");
