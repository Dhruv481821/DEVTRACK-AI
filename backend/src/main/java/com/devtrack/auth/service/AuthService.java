...
public class AuthService {
  ...
  /**
   * FR-AUTH-03 — rotation + reuse detection, per 12_Security.md §2.4. The reuse branch matters more
   * than the happy path: a revoked token being presented again means either a client retry bug or a
   * stolen token used after the legitimate client already rotated past it. Since those can't be
   * distinguished server-side, the safe default is revoking the entire session family.
   */
  @Transactional
    ...
    RefreshToken existing =
        refreshTokenRepository
            ...
            .orElseThrow(
                () ->
                    new AuthenticationException(
                        "AUTH_TOKEN_EXPIRED", "Refresh token is invalid or expired."));
  ...
  /**
   * FR-AUTH-05, step 1. Always succeeds from the caller's perspective — never reveals whether the
   * email exists.
   */
  @Transactional
    ...
    userRepository
        ...
        .ifPresent(
            user -> {
              ...
              String link =
                  emailProperties.frontend().baseUrl() + "/reset-password?token=" + rawToken;
              emailService.sendPasswordResetEmail(user.getEmail(), link);
            });
  }

  /**
   * FR-AUTH-05, step 2. Per 12_Security.md §2.5: a successful reset revokes every existing refresh
   * token for the user — a password reset is a reasonable signal that any existing session should
   * not be implicitly trusted to continue.
   */
  @Transactional
  ...
  /**
   * FR-AUTH-02 — Google OAuth path. Per that requirement's original acceptance criterion
   * ("downstream code never branches on auth method"), this converges on the same {@link
   * #issueTokenPair} used by password login.
   *
   * <p><strong>Real security tradeoff, flagged rather than silently decided:</strong> if a LOCAL
   * (password) account already exists with this email, this method logs into that existing account
   * via Google rather than rejecting or requiring explicit account-linking confirmation. This is a
   * common pattern (trusting Google's own email verification as equivalent to DevTrack's own), but
   * it does mean anyone who controls a given Google account can access a DevTrack account
   * registered with that same email via password — even without knowing the password. Acceptable
   * for v1 given Google verifies emails before allowing sign-in, but worth knowing this exists
   * rather than discovering it later. A stricter alternative (reject with a "sign in with password
   * instead" message, or require an explicit linking confirmation step) is a reasonable v2
  ...
  private void assignDefaultRole(AppUser user) {
    Role userRole =
        roleRepository
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException("USER role missing — check V1 migration seed data"));
  ...
  /**
   * Opaque, high-entropy, URL-safe — used for both refresh tokens and verification tokens. Never a
   * JWT (those are only for access tokens).
   */
  ...
  /**
   * SHA-256 of the raw token — the value actually persisted, per 12_Security.md §3: never store the
   * raw token.
   */
