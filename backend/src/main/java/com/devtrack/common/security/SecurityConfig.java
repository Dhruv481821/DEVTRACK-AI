...
/**
 * Implements /docs/07_Backend_Architecture.md §5 and /docs/12_Security.md §11:
 * stateless sessions (JWT per request, no server-side session state), strict CORS
 * origin allowlist (never a wildcard), no separate CSRF-token mechanism — the
 * SameSite=Strict refresh cookie + Bearer-header auth for everything else already
 * makes this API CSRF-immune by construction, per 12_Security.md §11's reasoning.
 */
@Configuration
...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable) // see class docblock — not needed given the token transport design
                ...
                .authorizeHttpRequests(
                        authorize ->
                                authorize
                                        ...
                                        .permitAll()
                                        // API docs — dev-profile only in practice (disabled in prod, application-prod.yml)
