...
public record ApiEnvelope<T>(boolean success, T data, ErrorBody error, Meta meta) {
    ...
    /**
     * Deliberately NOT an overload of success() — a second success(Page<T>) method
     * made every existing success(null) call site elsewhere in the codebase
     * ambiguous to the compiler (it couldn't tell whether `null` was meant to
     * resolve T against this overload or the plain one), breaking AuthController
     * and NotificationController's void-returning endpoints that were already
     * correct. A distinct name removes the ambiguity entirely instead of requiring
     * every call site to be rewritten with an explicit type witness.
     */
