...
/**
 * FR-GH-01. Plain REST calls to GitHub's OAuth endpoints — NOT Spring Security's
 * oauth2Login() — per the architectural note in this slice: connecting a secondary
 * account for an already-authenticated user is a different problem than
 * establishing identity via OAuth, and forcing it into the login-flow abstraction
 * would fight the framework rather than use it naturally.
 *
 * <p>Read-only-intent scope ("read:user repo") per 04_System_Architecture.md §4's
 * OAuth scope decision — worth a final check against GitHub's current documented
 * scope list at implementation time, since GitHub's exact scope granularity for
 * read-only repo access has changed across API versions historically.
 */
@Service
...
public class GithubOAuthService {
    ...
    /** Called by an authenticated endpoint — the state ties this specific user to the eventual callback. */
    ...
    /**
     * Called by the unauthenticated callback endpoint — GitHub redirects the
     * browser here with no DevTrack session/JWT attached, so the state parameter
     * (not the request's own auth) is what identifies the user.
     */
    @Transactional
    ...
    private String exchangeCodeForToken(String code) {
        ...
        if (accessToken == null) {
            throw new IllegalStateException(
                    "GitHub did not return an access_token: " + response);
        }
